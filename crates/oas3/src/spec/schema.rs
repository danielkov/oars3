...
/// Exclusive bound value for `exclusiveMinimum` and `exclusiveMaximum`.
///
/// In JSON Schema 2020-12 (OpenAPI 3.1), these are numbers. In JSON Schema draft-04 (OpenAPI
/// 3.0), these were booleans. This type accepts both for compatibility with specs in the wild.
#[derive(Debug, Clone, PartialEq, Deserialize, Serialize)]
#[serde(untagged)]
pub enum ExclusiveBound {
    /// JSON Schema 2020-12 / OpenAPI 3.1: the exclusive bound is a number.
    Number(serde_json::Number),

    /// JSON Schema draft-04 / OpenAPI 3.0: a boolean indicating whether the corresponding
    /// `minimum`/`maximum` is exclusive.
    Bool(bool),
}
...
pub struct ObjectSchema {
    ...
    ///
    /// In JSON Schema 2020-12 (OpenAPI 3.1), this is a number representing the exclusive upper
    /// limit. In JSON Schema draft-04 (OpenAPI 3.0), this was a boolean indicating whether
    /// `maximum` is exclusive. Both forms are accepted for compatibility.
    ///
    ...
    ///
    /// In JSON Schema 2020-12 (OpenAPI 3.1), this is a number representing the exclusive lower
    /// limit. In JSON Schema draft-04 (OpenAPI 3.0), this was a boolean indicating whether
    /// `minimum` is exclusive. Both forms are accepted for compatibility.
    ///
